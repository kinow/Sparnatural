...
(function( $ ) {
    ...
    HTMLElement.prototype.Sparnatural = function( options ) {
		...
		var defaults = {
			...
			onSubmit : null ,
			/**
			 * Callback notified when Sparnatural is redy for interaction.
			 * If function is difine on settings, she's will be executed before redyToInteract event is fired when UI/UX is loaded and displayed for user interaction.
			 *
			 * @param {object} form - The form object of Sparnatural
			 * @return {object} form - The form object of Sparnatural
			 **/
			readyToInteract : function(form) {
				console.log('Aucune définition personnalisée de définie d\'action à exécuter dès que le composant est prêt pour interactions avec l\'utilisateur') ;
				return form ;
			}
		...
		specProviderFactory.build(settings.config, settings.language, function(sp) {
			...
			//Sparnatural is ready to interact with user
			if (settings.readyToInteract instanceof Function) {
				thisForm = settings.readyToInteract(thisForm) ;
			}
			$(thisForm).trigger({type:"readyToInteract" }) ;
		});
